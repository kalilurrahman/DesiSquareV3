...
| Path | What it is |
|---|---|
| **`v4/`** | **v4 — the progressive increment of v1** (built on `phase1-mvp`, the app live at `desisquare-production`). A Reddit-calibre forum with **open user registration**, an **admin panel** (users/roles/spaces), **moderation** (review queue), **Reddit-style search** (All/Posts/Communities/Comments/Profiles + communities rail), and the **eToro-style maven percent-only performance proof** (Overview/Stats/Portfolio/Chart — asset value never shown). Runs standalone (`cd v4 && node server.mjs` → :8786) or against a **generic Discourse server** (`DISCOURSE_URL`). Deploy: `v4/DEPLOY.md`. |
| **`app/`** | The v2/v3-lineage product app (from v2 `desisquare-app`): zero-dependency single-file SPA + same-origin proxy. `cd app && node serve.mjs` → http://localhost:5191. Feed, maven profiles (Monthly/Yearly/Overall **% only**), member gains toggle (visible/private, % only), tabbed search (All/Communities/Posts/Comments/Profiles), moderation. Degrades gracefully to seeded content with all upstreams down. |
