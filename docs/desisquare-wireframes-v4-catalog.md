...
| **#9** *(new, v3)* | Recognition ranks engagement, never money. **Karma** (Epic 21), the leaderboard (W18), badges, streaks, and trending surfaces (W19, W3 rail) use participation signals only — gf-stats data is structurally unavailable to them. Karma chips, tier chips, and the 7.7 credibility strip carry no % returns and no currency. |
...
v3 five-tab search **plus**: `$NVDA` (any case) pins a **ticker chip** ("$NVDA · 14 discussions · label") linking to the W19 hub; label-name queries surface a **"Label: fema · view all"** suggestion chip → label browse; empty state shows trending ticker + label chips (4.5/20.2); Profile results carry **tier chips** (21.3). Operator queries (`tags:`, `#space`, `@author`) render as removable filter chips (4.8).

### W14 — Maven profile: performance proof *(● live)*
As v3 (percent-only, eToro-referenced, honest states, no leaderboards of returns — which is precisely what #9 now codifies platform-wide) **plus the five-signal credibility strip (7.7 · R5)** between the header and the privacy ribbon: Credential · Karma tier · Accepted answers · Tenure · Track-record proof. Flipping the W7 performance toggle changes **only** the proof signal to "not shared" — the other four stand (consent removes proof, never reputation).
...
Floating **💬 Chat** button (signed-in only, #7-A) → dock: channel list (US Square · CA Square · Ask-anything Lounge) with unread dots; pinned *"Education, not investment advice"* line (11.1); live messages with typing indicators; member send; threads affordance; **"⤴ Continue as discussion"** creates a real labelled topic with quoted pseudonym-only transcript and routes to W4 (17.3 — the anti-Discord-amnesia move, built on the native chat-transcript quote mechanic). Moderator sees a **freeze channel** (read-only) toggle (17.4 — core chat has no per-channel slow mode; pacing = channel status + trust-level rate limits). Chat is invisible signed-out and never mirrored to WhatsApp (#5).
...
### W18 — Recognition & Karma *(● live)* (Epics 18 + 21 · R5)
Route `#/recognition` + nav item "Leaderboard". Weekly/Monthly/All-time tabs; members ranked by **karma** (Epic 21 scoring — pill-weighted reactions + accepted answers; breakdown on hover); **tier chips** (New Arrival → Regular → Trusted → Anchor → Luminary); rank-change arrows; own row highlighted; standing banner: *"Ranks engagement — never portfolio performance (#9)"*. **"How karma works" transparency section (21.7):** weights table (Actionable/Helpful +3 · Insightful +2 · Like +1 · accepted answer +5 · upheld flag −5), tier ladder, anti-gaming rules, and a **dated weights changelog** — silent changes are a spec violation. **Desi badge ladder**: First Diya 🪔 · Neighbourly 🤝 · Straight Answer ✅ · Bridge Builder 🌉 · Square Pillar 🏛 with earned states. First reaction of the session fires a ≤2s **confetti micro-moment** + toast (reduced-motion honored) — the 18.3 celebration pattern. *(The parallel karma session's "W17" corresponds to this frame.)*
...
| R5 — karma (parallel build, remapped) | byline karma chips (W3/W4/W13/W16), W18 karma leaderboard + transparency, W14 five-signal strip (7.7), W3 top-contributors rail | Gamification scoring + Solved + scheduled badge queries; #9-bounded; leak-sweep covers karma payloads (12.5) |
