...
dev: ## Run the full local stack (app + 3 services) — Ctrl-C stops all
...
stop: ## Kill anything left listening on the stack ports
	@for p in 5191 8791 8788 8789; do pid=$$(lsof -ti tcp:$$p 2>/dev/null); [ -n "$$pid" ] && kill $$pid && echo "stopped :$$p" || true; done
